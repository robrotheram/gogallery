...
import (
	...
	"github.com/robrotheram/gogallery/backend/auth"
...
func InitApiRoutes(r *mux.Router, config *config.Configuration) *mux.Router {
	...
	r.Handle("/api/photos/latest", (getLatestCollectionsHandler)).Methods("GET")
	r.Handle("/api/photos/{date}", (getByDatePhotosHandler)).Methods("GET")
	...
	r.Handle("/api/admin/collection/{id}", auth.AuthMiddleware(updateCollectionHandler)).Methods("POST")
